...
[ROADMAP.md](../ROADMAP.md)의 챕터 6(`docs/ch-6/`, 인트로 1편과 본문 3편)을 집필하기 위한 상세 기획이다.
...
본문 3편은 콘텐츠 집필 지침의 기본 구조(학습 목표 → 배경 → 핵심 개념 → 실무 관점 → 더 깊이 → 정리 → 확인 문제 → 참고 자료)를 따르고, 45~90분 분량으로 작성한다. 00 인트로는 10~15분 분량의 오리엔테이션으로 구성한다.

### `00-introduction.md` — 코드와 운영체제 사이의 숨은 실행 계층

- **핵심 질문**: 같은 소스 코드가 런타임의 호출·할당·회수 정책에 따라 다른 크래시와 지연 특성을 보이는 이유는 무엇인가?
- 스택 트레이스, 메모리 증가, GC pause를 별개 증상이 아니라 런타임이 실행과 메모리를 관리한 결과로 보는 관점을 세운다.
- 호출과 스택 → 힙 할당과 소유권 → GC로 이어지는 학습 지도를 제시하고 내부 구조는 01~03에 위임한다.
...
1. 독자는 `00 → 01 → 02 → 03` 순서로 읽는다. 00이 문제의식과 학습 지도를 세우고, 주소 공간·스택 모델(01)이 힙 논의(02)의 좌표계이며, 할당자(02)가 GC(03)의 전제가 된다.
...
5. 문서 수나 완료 상태가 실제로 바뀔 때만 `PROGRESS.md`를 갱신한다. 실습 문서(`exercises/ch-6/`)는 인트로와 본문 3편 완료 후 §5 기획을 기준으로 작성한다.
