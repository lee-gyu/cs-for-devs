...
자료구조 책을 읽고 Big-O 표를 외웠다. 운영체제 강의에서 프로세스와 가상 메모리를 배웠다. 네트워크 책에서 TCP의 흐름 제어와 혼잡 제어도 읽었다. 그런데 운영 대시보드에서 CPU는 낮고 load average만 높은 상황을 보면 무엇부터 확인할지 떠오르지 않는다. 연결 리스트의 삽입이 O(1)이라는 문장은 기억하지만 실제 서비스에서 배열보다 나은 조건은 설명하기 어렵다.
...
“GC 때문에 느리다”는 이름 붙이기다. “할당률이 높아 young generation이 자주 차고, 승격된 객체가 old generation 압력을 높여 긴 수집이 요청 처리 스레드를 멈췄다”는 메커니즘 설명이다. 설명은 관찰한 지연과 힙·GC 상태 사이에 인과 경로를 제시한다.
...
- 스레드를 코어 수 이상으로 늘리면 처리량과 컨텍스트 스위치는 어떻게 변할 것인가?
...
모델이 정교해지면서 “write 성공은 내구성을 뜻한다”는 기존 가정이 어디서 틀렸는지 보인다. 모델에는 최소한 다음이 있어야 한다.
...
- 제목이 답할 것 같은 핵심 질문 한 개
- 현재 생각하는 실행 순서 또는 상태 변화
- 결과를 바꿀 것 같은 조건 세 개
- 확인하고 싶은 업무 사례 한 개

백지에서 시작하기 어렵다면 “A를 두 배로 늘리면 B는 어떻게 될까?” 형태로 쓴다. 예: “연결 풀을 두 배로 늘리면 처리량과 DB 락 대기는 어떻게 달라질까?”

### 읽는 중: 정의보다 인과 연결을 표시한다

용어마다 별도 노트를 만들기보다 다음 표시를 남긴다.
...
모든 문장을 요약하지 않는다. 이미 이해한 설명을 다시 옮기는 데 시간을 쓰기보다 예측을 바꾸는 정보에 집중한다.

### 읽은 후: 책을 닫고 복원한다

본문을 보지 않고 다음 네 작업을 한다.

1. 핵심 실행 흐름을 다시 그린다.
2. 출발 질문에 다섯 문장 이내로 답한다.
3. 조건 하나를 바꾼 새 사례의 결과를 예측한다.
4. 실험 또는 운영 지표 하나로 답을 검증한다.

설명할 수 없으면 문서를 처음부터 다시 읽지 않는다. 막힌 화살표에 해당하는 절만 찾아 모델을 수정한다. 확인 문제는 정답을 읽기 전에 근거와 반례까지 적는다.

### 며칠 뒤: 다른 문제에 전이한다

같은 예제를 반복하는 대신 표면이 다른 문제를 고른다.

- 해시 충돌을 배웠다면 메모이제이션 캐시의 키 분포를 분석한다.
- 큐 포화를 배웠다면 DB 연결 풀과 CI 작업 큐를 같은 모델로 비교한다.
- 그래프 도달성을 배웠다면 패키지 의존성, GC, 권한 상속을 각각 그래프로 모델링한다.
- 트랜잭션 격리를 배웠다면 재고 차감과 좌석 예약에서 필요한 불변식을 비교한다.

표면 용어를 걷어 냈을 때 같은 구조가 보이면 지식이 목록에서 모델로 바뀌고 있다는 신호다.
...
| 2주차 2일 | 무제한 큐, 빠른 실패, backpressure를 비교한다 | 트레이드오프 표 |
...
::: details 정답과 해설
먼저 질문이 사용자 공간 버퍼 복사 속도인지 영구 저장 장치의 내구 쓰기 처리량인지 고정한다. 파일 쓰기 모델에 페이지 캐시, dirty page, write-back, 장치 캐시와 완료 경계를 넣는다. `write` 반환만 기다리면 데이터가 페이지 캐시에 들어간 속도를 주로 측정했을 수 있다고 예측한다. 데이터 크기를 메모리보다 크게 하고 `fsync` 포함/제외, 동기 쓰기, direct I/O 같은 조건을 구분하며 장치 I/O와 dirty page 변화를 관찰한다. 파일 시스템·장치·가상화 환경을 기록한다. 결과는 틀린 숫자가 아니라 “내구성” 질문에 맞지 않는 측정일 수 있다.
:::
