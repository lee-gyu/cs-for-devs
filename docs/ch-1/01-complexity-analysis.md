...
캐시 계층이 왜 이런 배율을 만드는지(캐시 라인 크기, 프리페처, TLB)는 [7.2 메모리 계층과 캐시 일관성](../ch-7/02-memory-hierarchy.md)에서 하드웨어 메커니즘으로 다룬다. 챕터 1에서 필요한 것은 판단 기준이다: **접근 패턴이 순차인 자료구조와 포인터 추적인 자료구조는 점근 표기가 같아도 다른 부류로 취급해야 한다.**
